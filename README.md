...
If you would like to deploy your own version of the bot, it is designed to run on Heroku.  When running locally a `.env` file is used for the environment variables, including the Discord Token/ClientID, Daybreak Census API service ID, Postgres URL (optional), and Twitter bearer token (optional).  Subscription functionality will be disabled if a database URL is not present.
