			...
			try{
				...
				for(const cont of continents){
					if(territory[cont].locked != -1){
						...
						try{
							const result = await pgClient.query("SELECT u.channel, c.Indar, c.Hossin, c.Amerish, c.Esamir, c.autoDelete\
				...
				[territory["Indar"].locked == -1, territory["Hossin"].locked == -1, territory["Amerish"].locked == -1, territory["Esamir"].locked == -1, server]);
			}
			catch(err){
