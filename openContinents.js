...
const notifyUnlock = async function(cont, server, channelID, pgClient, discordClient){
	try{
		...
		if(typeof(channel.guild) !== 'undefined'){
			if(channel.permissionsFor(channel.guild.members.me).has([PermissionFlagsBits.SendMessages, PermissionFlagsBits.ViewChannel, PermissionFlagsBits.EmbedLinks])){
