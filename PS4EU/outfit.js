...
var q = async.queue(function(task, callback) {
	...
	uri = 'http://census.daybreakgames.com/s:'+process.env.serviceID+'/get/ps2ps4eu:v2/outfit?alias_lower='+oTag+'&c:resolve=member_character_name,member_online_status&c:join=character^on:leader_character_id^to:character_id';
	try{
		request(uri, function (error, response, body) {
			...
			if (data.outfit_list == null || data.returned == 0){
				...
				sendEmbed.setURL('https://ps4eu.ps2.fisu.pw/outfit/?name='+oTag);
				...
				try{
					...
					uri = 'https://census.daybreakgames.com/s:'+process.env.serviceID+'/get/ps2ps4eu:v2/character/'+resOut.leader_character_id+'?c:resolve=world'
